...
| 類型 | 實作方式 | 路徑範例 |
|---|---|---|
| 郵件開信追蹤 | 1×1 透明像素 | `/img/p.png?id=xxx`、`/track/open.gif` |
| 連結點擊追蹤 | 302 重新導向到原連結 | `/r/abc123`、`/l/xxx`、`/link/xxx` |
| **附件下載追蹤** | 反向代理附件串流 | `/att/xxx.pdf`、`/attachment/file` |

三種類型**共用同一套反向代理邏輯**,附件追蹤開箱即用、零額外設定 — Worker 透傳請求到你的後端,後端記錄事件後回傳檔案/像素/302。
