---
layout: home

hero:
  name: "Email Tracking Domain"
  text: "Ein-Klick E-Mail-Tracking-Domain"
  tagline: "Reverse-Proxy für Öffnungen / Klicks / Anhänge unter Ihrer eigenen Domain"
  actions:
    - theme: brand
      text: Ein-Klick-Deploy →
      link: /de/quick-start
    - theme: alt
      text: Eigene Domain
      link: /de/custom-domain

features:
  - icon: ⚡
    title: Ein-Klick-Deployment
    details: Cloudflare Workers / Vercel / Netlify / Deno Deploy — eine Plattform wählen, Deploy klicken.
  - icon: 🛡️
    title: Anti-Scanner eingebaut
    details: Erkennt automatisch Microsoft Defender SafeLinks Scanner für genaue Statistiken.
  - icon: 📎
    title: Drei Tracking-Typen sofort einsatzbereit
    details: Öffnungen + Link-Klicks + Anhang-Downloads, ohne zusätzliche Konfiguration.
  - icon: 🚀
    title: Globales Edge-CDN
    details: Alle 4 Plattformen laufen auf globalen Edge-Netzwerken — Millisekunden-Antwortzeit, keine Verzögerung beim Laden von E-Mail-Pixeln.
  - icon: 🌐
    title: Ihre eigene Domain
    details: track.yourdomain.com statt geteilter Domain für bessere Anti-Spam-Reputation.
  - icon: 🔓
    title: MIT Open Source
    details: Quellcode vollständig offen. Modifizierbar, selbst hostbar, kommerziell nutzbar.
---

<div style="text-align: center; margin: 64px 0 32px;">

## 🚀 Ein-Klick-Deployment auf Ihrer bevorzugten Plattform

</div>

<p style="text-align: center;">
<a href="https://deploy.workers.cloudflare.com/?url=https://github.com/suxuemi/email-track-domain" style="display: inline-flex; align-items: center; gap: 8px; padding: 12px 24px; margin: 4px 6px; background: #F38020; color: white; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 15px;"><img src="https://cdn.simpleicons.org/cloudflare/white" width="18" height="18" alt="">Auf Cloudflare deployen</a>
<a href="https://vercel.com/new/clone?repository-url=https://github.com/suxuemi/email-track-domain&root-directory=vercel&env=BACKEND_HOST,BACKEND_PROTOCOL,REDIRECT_TARGET&envDescription=Tracking+backend+host" style="display: inline-flex; align-items: center; gap: 8px; padding: 12px 24px; margin: 4px 6px; background: #000; color: white; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 15px;"><img src="https://cdn.simpleicons.org/vercel/white" width="18" height="18" alt="">Auf Vercel deployen</a>
<a href="https://app.netlify.com/start/deploy?repository=https://github.com/suxuemi/email-track-domain" style="display: inline-flex; align-items: center; gap: 8px; padding: 12px 24px; margin: 4px 6px; background: #00C7B7; color: white; text-decoration: none; border-radius: 6px; font-weight: 600; font-size: 15px;"><img src="https://cdn.simpleicons.org/netlify/white" width="18" height="18" alt="">Auf Netlify deployen</a>
</p>

<p style="text-align: center; margin-top: 32px;">
<a href="/de/quick-start">Auf weiteren Plattformen deployen →</a>
</p>
